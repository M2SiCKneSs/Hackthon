...
try:
    ...
    error_handler.handle_error("Error initializing server sockets", e)
    exit(1)
...
def send_offer():
    try:
        ...
        error_handler.handle_error("Error in send_offer,coudnlt send the offer", e)
...
def handle_single_udp_request(message, clientAddress):
    ...
    try:
        ...
        error_handler.handle_error(f"Error handling UDP request from {clientAddress}", e)
...
def handle_udp_requests():
    ...
    try:
        ...
        error_handler.handle_error("Error in handle_udp_requests", e)
        ...
        while True:
            ...
            def handle_client(client_socket, client_address):
                try:
                    ...
                    error_handler.handle_error(f"Error during communication with {client_address}", e)
                finally:
                    client_socket.close()

        Thread(target=handle_client, args=(client_socket, client_address), daemon=True).start()
    except Exception as e:
        error_handler.handle_error("Error in handle_tcp_clients", e)
